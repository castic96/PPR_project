...
double kiv_ppr_mapping::Band_Index_To_Level(const size_t index) {
    if (index == 0) {
        return kiv_ppr_mapping::Low_Threshold - kiv_ppr_mapping::Half_Band_Size;
    }
        
    if (index >= kiv_ppr_mapping::Band_Count - 1) {
        return kiv_ppr_mapping::High_Threshold + kiv_ppr_mapping::Half_Band_Size;
    }
        
    return kiv_ppr_mapping::Low_Threshold + static_cast<double>(index - 1)
        * kiv_ppr_mapping::Band_Size + kiv_ppr_mapping::Half_Band_Size;
}

size_t kiv_ppr_mapping::Band_Level_To_Index(double expected_value) {

    // Vraci index prvniho neuronu
    if (expected_value <= kiv_ppr_mapping::Low_Threshold) {
        return 0;
    }

    // Vraci index posledniho neuronu
    if (expected_value >= kiv_ppr_mapping::High_Threshold) {
    ...
    // TODO: mozna predelat
    double interval_ceiling = kiv_ppr_mapping::Low_Threshold;

    for (i = 1; i <= kiv_ppr_mapping::Internal_Bound_Count; i++) {

        interval_ceiling += kiv_ppr_mapping::Band_Size;
